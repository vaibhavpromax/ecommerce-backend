...
const addToCart = async (req, res) => {
  ...
  try {
    ...
    logger.info(`Cart ${created ? "created" : "exists"} `);
    if (quantity == 0) {
...
const getCart = async (req, res) => {
  ...
  try {
    ...
    logger.info("Fetched cart successfully");
    return successResponse(res, "Successfully fetched cart", cart);
  } catch (error) {
    logger.error(`Error while fetching cart ${error}`);
    return serverErrorResponse(res, "Error while fetching cart");
  }
};
